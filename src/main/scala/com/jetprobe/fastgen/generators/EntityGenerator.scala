...
/**
  * @author Shad.
  */
abstract class EntityGenerator(datasetConfig: Config,
                               regexMatcher: Regex.MatchIterator) {
  ...
  val datasetBuilder: Config => Map[String, Array[String]] =
    (config: Config) => {
      fieldOptions
        .filter(p => p._2.containsDataset)
        .map(x => {
          (x._2.getName,
           FileReader.getDataSet(
             config.getString(datasetPrefix + x._2.getName)))
        })
        .toMap
    }
  ...
  def addField(fieldName: String,
    ...
    val fieldOption = dataType match {
      case StringType =>
        (getRegex(fieldName), StringOption(getPattern(fieldName), fieldName))
      case IntType =>
        (getRegex(fieldName), IntOption(getPattern(fieldName), fieldName))
    }
  ...
  def transform(template: String): String = {
    ...
    fieldOptions.foreach {
      case (regex, opt) =>
        val generator = fieldGenHandlers.get(opt.getName)
...
case class IntOption(pattern: String,
                     fieldName: String,
                     minVal: Int = 0,
                     maxVal: Int = 100,
...
case class StringOption(pattern: String,
                        fieldName: String,
                        startsWith: Option[Regex] = None,
                        endsWith: Option[Regex] = None)
    extends FieldOption(pattern, fieldName)
